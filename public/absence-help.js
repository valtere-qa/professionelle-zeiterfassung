(() => {
  const addAbsenceHelp = () => {
    ...
    if (!grid.querySelector("[data-sync-help]")) {
      ...
      card.innerHTML = french
        ? "<span class='stable-help-number'>16</span><h3>Synchronisation et notifications</h3><p>Les données du profil sont synchronisées avec D1 en quelques secondes sur les appareils connectés.</p><div class='stable-help-steps'>À l’ouverture de l’application, la connexion est demandée. Après la connexion, les saisies, réglages, absences, calendrier, notes et rappels sont chargés puis synchronisés automatiquement. Utilisez la même URL de l’application et la même adresse e-mail sur tous les appareils. Lors du premier rapprochement, les données locales et celles du cloud sont réunies afin de ne rien perdre. Une modification venant d’un autre appareil est signalée avec son nom et propose « Actualiser maintenant ». Dans le menu du profil, activez les notifications du navigateur pour recevoir cette information lorsque l’application est ouverte. Après le premier profil, toute nouvelle inscription exige un code d’invitation à usage unique créé par un Owner, un Admin ou les RH. Le code est valable sept jours.</div>"
        : "<span class='stable-help-number'>16</span><h3>Synchronisation und Benachrichtigungen</h3><p>Die Profildaten werden innerhalb weniger Sekunden mit D1 auf den angemeldeten Geräten synchronisiert.</p><div class='stable-help-steps'>Beim Öffnen der App wird die Anmeldung verlangt. Nach der Anmeldung werden Buchungen, Einstellungen, Abwesenheiten, Kalender, Notizen und Erinnerungen geladen und automatisch synchronisiert. Verwende auf allen Geräten dieselbe App-URL und dieselbe E-Mail-Adresse. Beim ersten Abgleich werden bereits lokale und Cloud-Daten zusammengeführt, damit keine Einträge verloren gehen. Eine Änderung von einem anderen Gerät wird mit dessen Namen gemeldet und bietet „Jetzt aktualisieren“ an. Aktiviere im Profilmenü die Browser-Benachrichtigungen, damit diese Information bei geöffneter App zusätzlich angezeigt wird. Nach dem ersten Profil benötigt jede weitere Registrierung einen einmaligen Einladungscode von Owner, Admin oder HR. Der Code ist sieben Tage gültig.</div>";
      grid.append(card);
    }
  };
