(() => {
  const addAbsenceHelp = () => {
    ...
    if (!grid.querySelector("[data-sync-help]")) {
      ...
      card.innerHTML = french
        ? "<span class='stable-help-number'>15</span><h3>Synchronisation et notifications</h3><p>Les données du profil sont synchronisées avec D1 en quelques secondes sur les appareils connectés.</p><div class='stable-help-steps'>À l’ouverture de l’application, la connexion est demandée. Après la connexion, les saisies, réglages, absences, calendrier, notes et rappels sont chargés puis synchronisés automatiquement. Une modification venant d’un autre appareil est signalée avec son nom et propose « Actualiser maintenant ». Dans le menu du profil, activez les notifications du navigateur pour recevoir cette information lorsque l’application est ouverte. Après le premier profil, toute nouvelle inscription exige un code d’invitation à usage unique créé par un Owner, un Admin ou les RH. Le code est valable sept jours.</div>"
