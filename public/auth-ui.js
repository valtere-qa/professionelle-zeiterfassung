(() => {
  ...
  css.textContent=".auth-overlay{position:fixed;inset:0;background:#0b1f4788;z-index:1300;display:grid;place-items:center;padding:18px}.auth-card{background:#fff;border:1px solid #e1dfdd;border-radius:8px;max-width:460px;width:100%;padding:30px;box-shadow:0 12px 32px rgba(0,0,0,.18)}.auth-card h2{margin:0 0 8px;color:#12264a}.auth-card p{color:#73819a}.auth-tabs{display:flex;gap:6px;background:#eef3fb;padding:4px;border-radius:10px;margin:20px 0}.auth-tabs button{flex:1;border:0;background:transparent;padding:10px;border-radius:8px;font-weight:750}.auth-tabs button.active{background:#fff;color:#1769e8;box-shadow:0 2px 8px #12326318}.auth-field{margin:12px 0}.auth-field label{display:block;font-weight:750;margin-bottom:6px}.auth-field input{width:100%;padding:12px;border:1px solid #dbe3ee;border-radius:10px}.auth-error{color:#c94747!important;font-weight:650}.auth-info{background:#eef6ff;color:#245b9e;padding:12px;border-radius:10px}.auth-menu{position:absolute;left:18px;bottom:86px;width:270px;background:#fff;border:1px solid #e1dfdd;border-radius:8px;padding:12px;box-shadow:0 12px 32px rgba(0,0,0,.18);z-index:1250}.auth-menu button{width:100%;text-align:left;border:0;background:transparent;padding:11px;border-radius:9px;font-weight:700;color:#12264a}.auth-menu button:hover{background:#edebe9;color:#4f52a3}.auth-menu .auth-danger{color:#c94747}.auth-status{font-size:11px;color:#74839b;margin:5px 0 10px}.auth-quick{margin:10px 0 0;width:100%;display:block}.auth-quick.auth-logout{color:#c94747}@media(max-width:780px){.auth-quick{position:fixed;right:12px;top:12px;width:auto;z-index:1200;margin:0;background:#fff;box-shadow:0 8px 22px #12326322}}";
  ...
  const setProfile=(user)=>{
    ...
    const login=document.querySelector("#authLogin"); const register=document.querySelector("#authRegister"); const logoutButton=document.querySelector("#authLogout");
    if(login)login.hidden=Boolean(user); if(register)register.hidden=Boolean(user); if(logoutButton)logoutButton.hidden=!user;
  };
  const logout=async()=>{
  ...
  const showMenu=()=>{
    document.querySelector(".auth-menu")?.remove();const menu=document.createElement("div");menu.className="auth-menu";menu.innerHTML="<div class='auth-status' id='authStatus'>Status wird geprüft …</div><button id='authLogin'>↪ Einloggen</button><button id='authRegister'>＋ Registrierung</button><button id='authLogout' class='auth-danger' hidden>⇥ Abmelden</button>";document.body.append(menu);
    menu.querySelector("#authLogin").onclick=()=>{menu.remove();open("login")};menu.querySelector("#authRegister").onclick=()=>{menu.remove();open("register")};menu.querySelector("#authLogout").onclick=logout;
